/** Built-in themes. Each provides a value for every token in tokens.ts. */

export interface Theme {
  name: string;
  builtin?: boolean;
  tokens: Record<string, string>;
}

export const DARK: Theme = {
  name: "Тёмная",
  builtin: true,
  tokens: {
    "--ft-bg": "#0c0c0c",
    "--ft-surface": "#15161c",
    "--ft-surface-2": "#1a1b22",
    "--ft-surface-3": "#0d0d15",
    "--ft-border": "#252535",
    "--ft-border-strong": "#3a3a4a",
    "--ft-text": "#e0e0e0",
    "--ft-text-muted": "#9a9aae",
    "--ft-text-faint": "#5a5a6a",
    "--ft-text-on-accent": "#ffffff",
    "--ft-accent": "#3b82f6",
    "--ft-accent-hover": "#2f6fd6",
    "--ft-accent-soft": "rgba(59,130,246,0.15)",
    "--ft-success": "#22c55e",
    "--ft-success-soft": "rgba(34,197,94,0.15)",
    "--ft-danger": "#ef4444",
    "--ft-danger-hover": "#dc2626",
    "--ft-warning": "#f59e0b",
    "--ft-info": "#38bdf8",
    "--ft-header-bg": "#111217",
    "--ft-overlay": "rgba(0,0,0,0.6)",
    "--ft-shadow": "rgba(0,0,0,0.4)",
    "--ft-log-bg": "#07070b",
    "--ft-log-text": "#c8d0d8",
    "--ft-scrollbar": "#2a2a3a",
    "--ft-search-bg": "#1c1d30",
    "--ft-version-text": "#2b2d3a",
    "--ft-log-base": "#9ca3af",
    "--ft-log-crit": "#f87171",
    "--ft-log-err": "#f87171",
    "--ft-log-warn": "#fb923c",
    "--ft-log-ok": "#34d399",
    "--ft-log-okhi": "#22c55e",
    "--ft-log-dim": "#6b7280",
    "--ft-log-ts": "#555a66",
    "--ft-log-id": "#3b82f6",
    "--ft-log-dest": "#38bdf8",
    "--ft-glass-bg": "rgba(20,21,28,0.55)",
    "--ft-glass-bg-2": "rgba(24,25,32,0.88)",
    "--ft-glass-border": "rgba(255,255,255,0.08)",
    "--ft-glass-highlight": "rgba(255,255,255,0.12)",
    "--ft-glass-blur": "14px",
    "--ft-glass-shadow": "rgba(0,0,0,0.45)",
    "--ft-glow-1": "rgba(59,130,246,0.25)",
    "--ft-glow-2": "rgba(139,92,246,0.22)",
    "--ft-glow-3": "rgba(56,189,248,0.18)",
  },
};

export const LIGHT: Theme = {
  name: "Светлая",
  builtin: true,
  tokens: {
    "--ft-bg": "#f5f6f8",
    "--ft-surface": "#ffffff",
    "--ft-surface-2": "#ffffff",
    "--ft-surface-3": "#eef0f4",
    "--ft-border": "#d8dce4",
    "--ft-border-strong": "#b8bec8",
    "--ft-text": "#1a1d24",
    "--ft-text-muted": "#5a6270",
    "--ft-text-faint": "#9aa0ac",
    "--ft-text-on-accent": "#ffffff",
    "--ft-accent": "#2563eb",
    "--ft-accent-hover": "#1d4ed8",
    "--ft-accent-soft": "rgba(37,99,235,0.12)",
    "--ft-success": "#16a34a",
    "--ft-success-soft": "rgba(22,163,74,0.12)",
    "--ft-danger": "#dc2626",
    "--ft-danger-hover": "#b91c1c",
    "--ft-warning": "#d97706",
    "--ft-info": "#0284c7",
    "--ft-header-bg": "#ffffff",
    "--ft-overlay": "rgba(15,23,42,0.4)",
    "--ft-shadow": "rgba(20,30,50,0.12)",
    "--ft-log-bg": "#eef1f5",
    "--ft-log-text": "#1a1d24",
    "--ft-scrollbar": "#c4ccd6",
    "--ft-search-bg": "#ffffff",
    "--ft-version-text": "#cfd4dd",
    "--ft-log-base": "#334155",
    "--ft-log-crit": "#b91c1c",
    "--ft-log-err": "#dc2626",
    "--ft-log-warn": "#b45309",
    "--ft-log-ok": "#15803d",
    "--ft-log-okhi": "#16a34a",
    "--ft-log-dim": "#94a3b8",
    "--ft-log-ts": "#94a3b8",
    "--ft-log-id": "#2563eb",
    "--ft-log-dest": "#0891b2",
    "--ft-glass-bg": "rgba(255,255,255,0.62)",
    "--ft-glass-bg-2": "rgba(255,255,255,0.92)",
    "--ft-glass-border": "rgba(20,30,50,0.10)",
    "--ft-glass-highlight": "rgba(255,255,255,0.85)",
    "--ft-glass-blur": "16px",
    "--ft-glass-shadow": "rgba(20,30,50,0.14)",
    "--ft-glow-1": "rgba(37,99,235,0.16)",
    "--ft-glow-2": "rgba(14,165,233,0.14)",
    "--ft-glow-3": "rgba(99,102,241,0.13)",
  },
};

export const PINK: Theme = {
  name: "Розовая",
  builtin: true,
  tokens: {
    "--ft-bg": "#1a0f17",
    "--ft-surface": "#2a1622",
    "--ft-surface-2": "#331a2a",
    "--ft-surface-3": "#1f1019",
    "--ft-border": "#4a2540",
    "--ft-border-strong": "#6a3a5a",
    "--ft-text": "#ffe6f3",
    "--ft-text-muted": "#d8a8c4",
    "--ft-text-faint": "#a06a88",
    "--ft-text-on-accent": "#ffffff",
    "--ft-accent": "#ec4899",
    "--ft-accent-hover": "#db2777",
    "--ft-accent-soft": "rgba(236,72,153,0.18)",
    "--ft-success": "#34d399",
    "--ft-success-soft": "rgba(52,211,153,0.15)",
    "--ft-danger": "#fb7185",
    "--ft-danger-hover": "#f43f5e",
    "--ft-warning": "#fbbf24",
    "--ft-info": "#f0abfc",
    "--ft-header-bg": "#2a1622",
    "--ft-overlay": "rgba(26,15,23,0.7)",
    "--ft-shadow": "rgba(0,0,0,0.45)",
    "--ft-log-bg": "#150b11",
    "--ft-log-text": "#f0d0e2",
    "--ft-scrollbar": "#4a2540",
    "--ft-search-bg": "#3a2030",
    "--ft-version-text": "#492f3f",
    "--ft-log-base": "#f0d0e2",
    "--ft-log-crit": "#fb7185",
    "--ft-log-err": "#fb7185",
    "--ft-log-warn": "#fbbf24",
    "--ft-log-ok": "#34d399",
    "--ft-log-okhi": "#4ade80",
    "--ft-log-dim": "#a06a88",
    "--ft-log-ts": "#8a5a74",
    "--ft-log-id": "#f0abfc",
    "--ft-log-dest": "#f9a8d4",
    "--ft-glass-bg": "rgba(42,22,34,0.52)",
    "--ft-glass-bg-2": "rgba(46,24,38,0.88)",
    "--ft-glass-border": "rgba(255,210,235,0.12)",
    "--ft-glass-highlight": "rgba(255,210,235,0.16)",
    "--ft-glass-blur": "14px",
    "--ft-glass-shadow": "rgba(0,0,0,0.45)",
    "--ft-glow-1": "rgba(236,72,153,0.28)",
    "--ft-glow-2": "rgba(240,171,252,0.22)",
    "--ft-glow-3": "rgba(251,113,133,0.20)",
  },
};

export const MIDNIGHT_DECO: Theme = {
  name: "Midnight Deco",
  builtin: true,
  tokens: {
    "--ft-bg": "#10131c",
    "--ft-surface": "#161b27",
    "--ft-surface-2": "#181e2c",
    "--ft-surface-3": "#1d2433",
    "--ft-border": "#2a3346",
    "--ft-border-strong": "#3d4a63",
    "--ft-text": "#ede4d3",
    "--ft-text-muted": "#9aa2b8",
    "--ft-text-faint": "#5a6378",
    "--ft-text-on-accent": "#10131c",
    "--ft-accent": "#d4af6a",
    "--ft-accent-hover": "#e6c583",
    "--ft-accent-soft": "rgba(212,175,106,0.14)",
    "--ft-success": "#5fae8f",
    "--ft-success-soft": "rgba(95,174,143,0.14)",
    "--ft-danger": "#c1644f",
    "--ft-danger-hover": "#a8503e",
    "--ft-warning": "#d4af6a",
    "--ft-info": "#6e93b8",
    "--ft-header-bg": "#161b27",
    "--ft-overlay": "rgba(8,10,16,0.6)",
    "--ft-shadow": "rgba(0,0,0,0.4)",
    "--ft-log-bg": "#12161f",
    "--ft-log-text": "#ede4d3",
    "--ft-scrollbar": "#3d4a63",
    "--ft-search-bg": "#181e2c",
    "--ft-version-text": "#3d4a63",
    "--ft-log-base": "#9aa2b8",
    "--ft-log-crit": "#a8503e",
    "--ft-log-err": "#c1644f",
    "--ft-log-warn": "#d4af6a",
    "--ft-log-ok": "#5fae8f",
    "--ft-log-okhi": "#7bc4a4",
    "--ft-log-dim": "#454e63",
    "--ft-log-ts": "#5a6378",
    "--ft-log-id": "#d4af6a",
    "--ft-log-dest": "#6e93b8",
    "--ft-glass-bg": "rgba(22,27,39,0.62)",
    "--ft-glass-bg-2": "rgba(22,27,39,0.92)",
    "--ft-glass-border": "rgba(212,175,106,0.16)",
    "--ft-glass-highlight": "rgba(237,228,211,0.08)",
    "--ft-glass-blur": "16px",
    "--ft-glass-shadow": "rgba(0,0,0,0.35)",
    "--ft-glow-1": "rgba(212,175,106,0.12)",
    "--ft-glow-2": "rgba(110,147,184,0.10)",
    "--ft-glow-3": "rgba(95,174,143,0.10)",
  },
};

export const BUILTIN_THEMES: Theme[] = [DARK, LIGHT, PINK, MIDNIGHT_DECO];
